...
/**
 * Middleware that sends an error if the requesting user is not a mod and not the owner of the target team.
...
function assertUserTargetTeamOwner(
  ...
  if (
    !res.locals.user.mod &&
    res.locals.targetTeam.ownerId != res.locals.user
  ) {
    res.status(401).send("User is not the team owner and is not a mod.");
    return;
  }
